...
std::string getDavidMicrobenchmarkProfilePath() {
  namespace fs = std::filesystem;
  fs::path src = fs::path(__FILE__).parent_path().parent_path().parent_path();
  fs::path profile = src / "backend" / "costmodel_profiles" / "microbench" /
...
TEST(CostModelHardwareConfigTest,
  ...
  const auto *measurement =
      profile->getMeasurement("simd.f32.add.throughput");
  ...
  EXPECT_EQ(measurement->scope, "single_aiv");
  ...
  auto wrongDomain = profile->requireValue(

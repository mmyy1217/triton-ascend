...
SimdSimtFeatureSummary triangularUnknownLoopFeatures() {
  ...
  // The TTIR analysis reports two sibling block-update loops whose trip
  // counts are dynamic (min(T, 16), ...).  This is the only bounded exception
  // to the generic unknown-loop calibration rejection.
  f.staticLoopCount = 0;
  f.staticLoopTripCountSum = 0;
  f.staticLoopTripCountMax = 1;
...
TEST(SimdSimtCostModelTest,
  ...
  EXPECT_NEAR(report->candidateCosts.allSimtOnly, 1408.4638112636317,
              1.0e-6);
  EXPECT_DOUBLE_EQ(
      report->uncalibratedCandidateCosts.mixedSimdSimt,
  ...
  EXPECT_DOUBLE_EQ(report->eventRouteScoreMultipliers.mixedSimdSimt,
                   0.666478);
  ...
  EXPECT_EQ(report->profileVersion,
            "david-v100-simd-simt-20260804-v10");
...
TEST(SimdSimtCostModelTest,
     FbgemmGoldenScoresRequireMaterializableMixedPlan) {
...
TEST(SimdSimtCostModelTest,
  ...
  modelOptions.scoreOutsideCalibrationCoverage = false;
  auto report = estimateSimdSimtCandidates(
      triangularUnknownLoopFeatures(), modelOptions);
  if (!report)
    FAIL() << llvm::toString(report.takeError());
...
TEST(SimdSimtCostModelTest,
  ...
  auto oversizedReport =
      estimateSimdSimtCandidates(oversized, modelOptions);
  if (!oversizedReport)
    FAIL() << llvm::toString(oversizedReport.takeError());
  EXPECT_FALSE(oversizedReport->calibrationCovered);
  EXPECT_EQ(oversizedReport->calibrationDomain,
            "unknown_loop_trip_count");
}

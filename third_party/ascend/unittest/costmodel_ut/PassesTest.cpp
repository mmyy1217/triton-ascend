...
module {
  ...
  const auto *facts =
      std::get_if<mlir::ascend::HistogramFacts>(&anchor.facts);
...
module {
  func.func @main(%input: tensor<16xi32>, %state: tensor<16x16xf32>)
    ...
    %a = scf.for %i = %c0 to %c16 step %c1 iter_args(%acc = %state)
        -> (tensor<16x16xf32>) {
      ...
      %sel = arith.select %mask, %acc, %acc : tensor<16x16xi1>, tensor<16x16xf32>
      scf.yield %sel : tensor<16x16xf32>
    }
    %b = scf.for %i = %c0 to %c16 step %c1 iter_args(%acc = %a)
        -> (tensor<16x16xf32>) {
      %load = "tt.load"(%input) : (tensor<16xi32>) -> tensor<16xf32>
      %red = "tt.reduce"(%acc) ({
      ^bb0(%lhs: f32, %rhs: f32):
  ...
  EXPECT_EQ(plan.materializableCount(), 2);

...
def _load_route_report(path, expected):
    ...
    assert len(manifests) == 1, f"expected one StageModel snapshot below {path}"
    snapshot = manifests[0].parent
    report = json.loads((snapshot / "summary.json").read_text())
    features = json.loads((snapshot / "features.json").read_text())
    config = json.loads((snapshot / "config.json").read_text())
    report["stage_model"] = json.loads((snapshot / "stage-boundary-graph.json").read_text())
    report["features"] = features["features"]
    report["route_transform_capability"] = config["route_transform_capability"]
...
def test_costmodel_gather_dot_min(tmp_path):
    ...
    report_path = tmp_path / "gather_dot_min_snapshots"
...
def test_costmodel_solve_tril(tmp_path):
    ...
    report_path = tmp_path / "solve_tril_snapshots"
...
def test_costmodel_fbgemm_rowwise_quant(tmp_path):
    ...
    report_path = tmp_path / "fbgemm_snapshots"

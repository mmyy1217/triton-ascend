...
MicrobenchmarkProfile::loadFromFile(llvm::StringRef path) {
  ...
  if (!buffer)
    return llvm::createStringError(
        buffer.getError(), "failed to read microbenchmark profile '%s'",
        path.str().c_str());
  ...
  if (!parsed)
    return llvm::createStringError(
        ...
        "failed to parse microbenchmark profile '%s': %s",
        path.str().c_str(), llvm::toString(parsed.takeError()).c_str());
...
llvm::Expected<double> MicrobenchmarkProfile::requireValue(
    llvm::StringRef key,
    std::optional<llvm::StringRef> expectedUnit,
  ...
  if (expectedCycleDomain &&
      measurement->cycleDomain != *expectedCycleDomain)
    return llvm::createStringError(
        std::errc::invalid_argument,

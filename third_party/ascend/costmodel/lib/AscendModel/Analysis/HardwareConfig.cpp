...
HardwareConfig::loadFromJSON(const llvm::json::Value &json) {
  ...
  if (!config->microbenchmarkProfileReference.empty()) {
    ...
    if (!config->loadReferencedMicrobenchmarkProfile(/*configPath=*/"", error)) {
      llvm::errs() << "Error loading shared microbenchmark profile: " << error
                   << "\n";
      return nullptr;
    }
...
bool HardwareConfig::loadReferencedMicrobenchmarkProfile(
  ...
  if (!target.empty() &&
      llvm::StringRef(target) != profile->getTarget()) {
    error = "shared microbenchmark target '" +
            profile->getTarget().str() +
            "' does not match hardware config target '" + target + "'";
    return false;
  }

  const double configuredDeviceMHz = clockFreqGHz * 1000.0;
  const double clockToleranceMHz =
      std::max(1.0, configuredDeviceMHz * 1.0e-3);
  if (configuredDeviceMHz <= 0.0 ||
      std::abs(*measuredDeviceMHz - configuredDeviceMHz) >
          clockToleranceMHz) {
  ...
  auto f32AddThroughput = profile->requireValue(
      "simd.f32.add.throughput", "vector_instruction/system_cycle",
      "SYS_CNT");
  if (!f32AddThroughput) {
  ...
  if (*f32AddThroughput <= 0.0) {
    error =
        "shared microbenchmark SIMD f32.add throughput must be positive";
    return false;
  }
...
double HardwareConfig::getMicrobenchmarkRatePerDeviceCycle(

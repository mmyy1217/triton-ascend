...
static void eraseAngleBracketClause(std::string &line,
                                    llvm::StringRef name) {
  std::string needle = (name + " = <").str();
  for (;;) {
    size_t pos = line.find(needle);
...
void HIVMAnalysisReport::emitFeedbackJSON(
    llvm::raw_ostream &os, const HardwareConfig &config) const {
  ...
  quality["fallback_operation_count"] =
      static_cast<int64_t>(unknownOpCount);
  ...
  summary["critical_path_us"] =
      config.cyclesToMicroseconds(oneIterationCycles);
  ...
  root["scheduler"] =
      HIVMAnalyzer::stringifySchedulerMode(schedulerMode).str();

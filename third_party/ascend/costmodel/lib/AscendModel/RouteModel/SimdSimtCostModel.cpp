...
loadCandidateProfile(llvm::StringRef requestedPath) {
  ...
  if (calibration) {
    ...
    if (calibration->get("event_route_score_multiplier")) {
      ...
      if (const auto *domains =
              reader.object(*eventCalibration, "domains",
        ...
        for (const auto &entry : *domains) {
          ...
          route.allSimdMultiplier =
              reader.number(*domain, "all_simd", context);
          ...
          auto allSimtValidated =
              domain->getBoolean("all_simt_only_validated");
          if (!allSimtValidated) {
          ...
          auto mixedValidated =
              domain->getBoolean("mixed_simd_simt_validated");
          if (!mixedValidated) {
            reader.setError(context +
  ...
  if (simt) {
    ...
    if (usesAnchorPartitionProfile)
      mixedSetupFallback =
          reader.object(*simt, "mixed_setup_fallback", "simt");
    else
    ...
    if (mixedSetupFallback) {
      ...
      profile.mixedSetupFallbackConfidence = reader.optionalString(
          *mixedSetupFallback, "confidence",
  ...
  if (profile.profileVersion != "david-v100-simd-simt-20260727-v3" &&
    ...
    return llvm::createStringError(
        ...
        "(expected v3, v4, v5, v6, v7, v8, v9, or v10)",
  ...
  if (usesSharedMicrobench &&
    ...
    return llvm::createStringError(
        std::errc::invalid_argument,
        "SIMD/SIMT v5/v6/v7/v8/v9/v10 profile must reference "
  ...
  if (usesSharedMicrobench &&
      ((!usesAnchorPartitionProfile && *selectionSchemaVersion != 2) ||
       (!usesEventCalibrationSchema &&
        usesAnchorPartitionProfile && *selectionSchemaVersion != 3) ||
       (usesEventCalibrationSchema &&
        *selectionSchemaVersion != 4)))
    return llvm::createStringError(
        std::errc::invalid_argument,
        "SIMD/SIMT v5/v6 requires schema_version 2 and v7 requires "
        "schema_version 3; v9/v10 require schema_version 4");
  if (profile.simdVectorWidthBits <= 0 || profile.simtWarpSize <= 0 ||
    ...
    return llvm::createStringError(
        ...
        "fallbacks");
  if (usesEventCalibrationSchema &&
      profile.eventRouteCalibration.empty())
    return llvm::createStringError(
        std::errc::invalid_argument,
        "SIMD/SIMT v9/v10 profile requires event route calibration domains");
  for (const auto &entry : profile.eventRouteCalibration) {
    ...
    if (!std::isfinite(route.allSimdMultiplier) ||
        ...
        route.allSimdMultiplier <= 0.0 ||
        route.allSimtOnlyMultiplier <= 0.0 ||
...
legalCandidates(const SimdSimtCandidateScores &scores, bool allSimdLegal,
  ...
  if (allSimdLegal)
    candidates.push_back(
        {scores.allSimd, SimdSimtCandidateKind::AllSIMD});
  if (allSimtLegal)
    candidates.push_back(
...
chooseBest(const SimdSimtCandidateScores &scores, bool allSimdLegal,
           bool allSimtLegal, bool mixedLegal) {
...
static llvm::json::Array toReasonJSON(
...
toLowerabilityJSON(const CandidateLowerability &lowerability) {
  ...
  result[kAllSimd] =
      route(lowerability.allSimd, lowerability.allSimdReasons);
  result[kAllSimtOnly] =
      route(lowerability.allSimtOnly,
            lowerability.allSimtOnlyReasons);
  result[kMixedSimdSimt] =
      route(lowerability.mixed, lowerability.mixedReasons);
  return result;
}

static llvm::json::Object
toAtomicFactsJSON(const TensorAtomicFacts &facts) {
  llvm::json::Object result;
  ...
  if (facts.staticMaskActiveFraction)
    result["static_mask_active_fraction"] =
  ...
  result["address_depends_on_loaded_index"] =
      facts.addressDependsOnLoadedIndex;
  result["contention"] = facts.contention;
  return result;
}

static llvm::json::Object
toHistogramFactsJSON(const HistogramFacts &facts) {
  llvm::json::Object result;
...
llvm::json::Object SimtAnchorFeatureSummary::toJSON() const {
  ...
  result["kernel_lowerability"] =
      toLowerabilityJSON(kernelLowerability);
  return result;
}
...
llvm::json::Object SimdSimtCostBreakdown::toJSON(
  ...
  mixed["mixed_setup_fallback_system_cycles"] =
      mixedSetupFallbackCycles;
  ...
  partition["simd_regular_dot_system_cycles"] =
      mixedSimdRegularDotCycles;
  partition["simd_regular_memory_system_cycles"] =
      mixedSimdRegularMemoryCycles;
  partition["simd_regular_payload_system_cycles"] =
      mixedSimdRegularPayloadCycles;
  partition["simt_anchor_compute_system_cycles"] =
      mixedSimtAnchorComputeCycles;
  partition["simt_anchor_dot_system_cycles"] =
      mixedSimtAnchorDotCycles;
  partition["simt_anchor_memory_system_cycles"] =
      mixedSimtAnchorMemoryCycles;
  partition["simt_anchor_shuffle_system_cycles"] =
  ...
  partition["applied_boundary_fallback_system_cycles"] =
      mixedBoundaryCycles;
...
llvm::json::Object SimdSimtCostReport::toJSON() const {
  ...
  eventCalibration["domain"] =
      eventRouteCalibrationApplied ? llvm::json::Value(calibrationDomain)
                                   : llvm::json::Value(nullptr);
  eventCalibration["all_simt_only_validated"] =
      eventAllSimtOnlyValidated;
  eventCalibration["mixed_simd_simt_validated"] =
      eventMixedSimdSimtValidated;
  ...
  if (candidateCostsEvaluated) {
    ...
    eventCalibration["score_multipliers"] =
        eventRouteScoreMultipliers.toJSON();
  } else {
  ...
  if (candidateCostsEvaluated) {
    ...
    if (auto *structureObject = result.getObject("structure")) {
      ...
      (*structureObject)["calibration_sample_domain"] =
          calibrationDomain;
    }
  } else {
...
mlir::ascend::analyzeSimdSimtFeatures(
  ...
  llvm::SmallVector<Operation *> anchorRoots =
      anchorPlan.materializableRoots();
  llvm::DenseSet<Operation *> anchorSet(anchorRoots.begin(),
                                        anchorRoots.end());
  features.simtAnchors.recognizedCount = anchorPlan.anchors.size();
  features.simtAnchors.count = anchorRoots.size();
  features.simtAnchors.kernelLowerability =
      anchorPlan.kernelLowerability;
  ...
  for (Operation *anchor : anchorRoots) {
    ...
    for (Value result : anchor->getResults()) {
      if (result.use_empty() || !isa<RankedTensorType>(result.getType()) ||
          !escapingTensors.insert(result).second)
        continue;
  ...
  auto updateTypeStats = [&](Type type, bool inAnchor) {
    if (auto tensor = dyn_cast<RankedTensorType>(type)) {
      ...
      if (inAnchor) {
        features.simtAnchors.maxElementBits =
            std::max(features.simtAnchors.maxElementBits,
                     getTypeBitWidth(type));
        features.simtAnchors.maxTensorNumel =
            std::max(features.simtAnchors.maxTensorNumel,
                     getStaticNumElements(type));
      }
    }
  };
  ...
  module.walk([&](Operation *op) {
    ...
    if (name == "tt.load")
      ...
      incrementRaw(features.histogramOps,
                   features.simtAnchors.histogramOps);
    ...
    if (!weightedKind.empty()) {
      ...
      features.opElements[weightedKind] +=
    ...
    if (name == "tt.dot" && op->getNumOperands() >= 2) {
      ...
      if (lhs && rhs && lhs.getRank() >= 2 && rhs.getRank() >= 2) {
        ...
        if (m > 0 && n > 0 && k > 0) {
          ...
          if (inAnchor)
            features.simtAnchors.dotFlops +=
                2 * m * n * k * loopMultiplier;
        }
      }
    }
        ...
        if (source && source.hasStaticShape() && axis) {
          ...
          if (axisValue >= 0 && axisValue < source.getRank()) {
            ...
            if (extent > 0) {
              ...
              features.weightedReduceAxisElements +=
                  extent * loopMultiplier;
              if (inAnchor) {
                features.simtAnchors.maxReduceAxisExtent =
                    std::max(features.simtAnchors.maxReduceAxisExtent,
                             extent);
    ...
    auto recordUniqueMask = [&](Value value) {
      ...
      if (uniqueMasks.insert(value).second) {
        ...
        features.predicateElements += getStaticNumElements(type);
      }
      if (inAnchor && anchorUniqueMasks.insert(value).second) {
        ...
        features.simtAnchors.predicateElements +=
            getStaticNumElements(type);
      }
    };
...
mlir::ascend::estimateSimdSimtCandidates(
  ...
  report.mixedCandidateLegal =
      !features.hasExplicitScope && report.applicability.materializable &&
  ...
  const int64_t anchorWeightedScans =
      mapValue(features.simtAnchors.weightedOps, "scan",
  ...
  if (!nearestSetupFallback)
    return llvm::createStringError(std::errc::invalid_argument,
                                   ...
                                   "fallback");
  report.breakdown.mixedSetupFallbackNumWarps =
      nearestSetupFallback->numWarps;
  ...
  // A mixed route is not a convex blend of two whole-kernel costs.  Such a
  // blend can never beat both endpoints once transition setup is added.
  // Instead, charge the exact materializable anchors at SIMT rates and the
  // remaining operations at SIMD rates.  The two phases execute
  // sequentially, so their resource rooflines are added.
  ...
  report.breakdown.mixedSimtAnchorPayloadCycles =
      std::max(report.breakdown.mixedSimtAnchorComputeCycles +
                   report.breakdown.mixedSimtAnchorDotCycles +
                   report.breakdown.mixedSimtAnchorShuffleCycles,
               report.breakdown.mixedSimtAnchorMemoryCycles) +
      report.breakdown.mixedSimtAnchorPredicateCycles;
  ...
  const int64_t remainingMaskRank =
      std::max<int64_t>(0, features.maskRankSum -
                               features.simtAnchors.maskRankSum);
  const int64_t remainingWeightedReductions =
      std::max<int64_t>(0, weightedReductions -
                               anchorWeightedReductions);
  ...
  report.breakdown.mixedSimdFraction =
      totalPartitionWork > 0.0
          ? std::clamp(1.0 - anchorPartitionWork / totalPartitionWork,
                       0.0, 1.0)
          : 0.0;
  ...
  report.decision =
      chooseBest(report.candidateCosts,
                 report.allSimdCandidateLegal,
                 report.allSimtOnlyCandidateLegal,
                 report.mixedCandidateLegal);
  report.runnerUp =
      chooseRunnerUp(report.candidateCosts,
                     report.allSimdCandidateLegal,
                     report.allSimtOnlyCandidateLegal,
  ...
  const double gainBaseline =
      report.allSimdCandidateLegal ? report.candidateCosts.allSimd
